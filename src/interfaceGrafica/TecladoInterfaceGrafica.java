...
public class TecladoInterfaceGrafica {
    ...
    public TecladoInterfaceGrafica(String instrumentoNome) {
        ...
        try {
            File arquivoMapNotas = new File("./assets/mapeamento_" + instrumentoNome + ".txt"); // arquivo texto de mapeamento de notas
            ...
            while (leitorLinha.hasNextLine()) {
                ...
                char oitavaString = partes[1].charAt(partes[1].length() - 1);
                int oitava = oitavaString - '0';
                if (oitava < menorOitava) {
                    menorOitava = oitava;
                }
        ...
        for (Tecla tecla : NOTAS.keySet()) {
            ...
            TECLAS.put(tecla ,new TeclaInterfaceGrafica(nota.substring(0, nota.length() - 1), nota.charAt(nota.length() - 1) - '0' - menorOitava));
        }
